    ...
    <table cellspacing="0" cellpadding="0" class="TableStyle">
        ...
        <s:iterator value="list" status="status">
			<tr class="TableDetail1 template">
				...
				<td>${description}&nbsp;</td>
				<td>
					<s:a action="forumManage_delete?forum.id=%{id}" onclick="return delConfirm()">删除</s:a>
					<s:a action="forumManage_editUI?forum.id=%{id}">修改</s:a>
					...
					<s:else>
						<s:a action="forumManage_moveUp?forum.id=%{id}">上移</s:a>
					</s:else>
					...
					<s:else>
						<s:a action="forumManage_moveDown?forum.id=%{id}">下移</s:a>
					</s:else>
